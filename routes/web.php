...
Route::get('/', function () {
    if (app()->environment('production') && !env('APP_INSTALLED', false)) {
...
// Simple web-based installer (only when app is not yet installed)
if (!env('APP_INSTALLED', false)) {
    Route::get('/install', function () {
        ...
        $allOk = $requirements['php_version']['ok']
        ...
        return Inertia::render('Install', [
            ...
            'canContinue' => $allOk,
            'envExampleExists' => File::exists(base_path('.env.example')),
        ]);
    })->name('install.index');

    Route::post('/install/environment', function (\Illuminate\Http\Request $request) {
        ...
        foreach ($replacements as $key => $value) {
            if (preg_match("/^{$key}=.*/m", $base)) {
                $base = preg_replace("/^{$key}=.*/m", $key.'='.$value, $base);
            } else {
                $base .= PHP_EOL.$key.'='.$value;
            }
        }

        File::put($envPath, $base);

        return back()->with('success', 'Environment configuration saved.');
    })->name('install.environment');

    Route::post('/install/migrate', function () {
        try {
    ...
    Route::post('/install/admin', function (\Illuminate\Http\Request $request) {
        $data = $request->validate([
        ...
        // Ensure migrations have run (users table exists)
        if (!\Schema::hasTable('users')) {
            return back()->with('error', 'Users table not found. Please run migrations first.');
        }
        ...
        if (File::exists($envPath)) {
            $base = File::get($envPath);
            if (preg_match('/^APP_INSTALLED=.*/m', $base)) {
                $base = preg_replace('/^APP_INSTALLED=.*/m', 'APP_INSTALLED=true', $base);
            } else {
                $base .= PHP_EOL.'APP_INSTALLED=true';
            }
            File::put($envPath, $base);
        }

        return redirect()->route('login')->with('success', 'Admin user created. You can now log in.');
    })->name('install.admin');
}
